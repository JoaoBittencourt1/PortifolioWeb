...
function PixelSwap({
  ...
  // Upstream React Bits pattern: latest config read from effects without re-running them.
  ...
  useEffect(() => {
    if (transition) {
      // Local change: if the target flips back mid-swap (e.g. the pointer left), cancel
      // instead of finishing and then swapping back on its own.
      if (desiredActive !== transition.to) {
        stopAnimations();
        // eslint-disable-next-line react-hooks/set-state-in-effect -- same effect-driven pattern as the upstream swap start
        setTransition(null);
      }
      return;
    }
